                ...
                with gr.Blocks():
                    ...
                    with gr.Row():
                        with gr.Column():
                            ...
                            shared.gradio['exl3_image_cache_mib'] = gr.Number(label='Image embedding cache (MiB)', value=shared.args.exl3_image_cache_mib, minimum=0, precision=0, interactive=not mu, info='Reuse unchanged images across requests. Bounded memory; 0 disables. Takes effect on model reload.')
                            shared.gradio['exl3_max_chunk_size'] = gr.Number(label='Prompt processing chunk size', value=shared.args.exl3_max_chunk_size, minimum=256, step=256, precision=0, interactive=not mu, info='ExLlamaV3: 2048 is the default. Larger chunks may improve long-prompt processing but need more temporary VRAM. Takes effect on model reload.')

                            shared.gradio['exl3_recurrent_cache_mib'] = gr.Number(label='Recurrent checkpoint RAM (MiB)', value=shared.args.exl3_recurrent_cache_mib, minimum=1, precision=0, interactive=not mu, info='System RAM budget, not VRAM. More space retains more conversation checkpoints. Requires reload.')
                            shared.gradio['exl3_recurrent_checkpoint_interval'] = gr.Number(label='Tail/decode checkpoint interval', value=shared.args.exl3_recurrent_checkpoint_interval, minimum=0, step=256, precision=0, interactive=not mu, info='Tokens between checkpoints near the prompt tail and during output. 0 uses the architecture default. Requires reload.')
                            shared.gradio['exl3_recurrent_checkpoint_interval_pp'] = gr.Number(label='Prefill checkpoint interval', value=shared.args.exl3_recurrent_checkpoint_interval_pp, minimum=256, step=256, precision=0, interactive=not mu, info='Smaller intervals can reduce branch replay, but add RAM copies. Rounded up to a prompt chunk. Requires reload.')
                        ...
                        with gr.Column():
                            ...
                            with gr.Accordion("Speculative decoding", open=False) as shared.gradio['speculative_decoding_accordion']:
                                ...
                                shared.gradio['exl3_dynamic_draft'] = gr.Checkbox(label='Adaptive draft length (ExLlamaV3)', value=shared.args.exl3_dynamic_draft, interactive=not mu, info='Shorten low-confidence drafts automatically, up to draft-max. Requires a draft model or built-in MTP head. Takes effect on model reload.')
                                shared.gradio['exl3_draft_confidence'] = gr.Slider(label='Adaptive draft acceptance target', minimum=0.05, maximum=0.95, step=0.05, value=shared.args.exl3_draft_confidence, interactive=not mu, info='ExLlamaV3 only. Default 0.40. Higher values shorten drafts more aggressively. Ignored when adaptive drafting is off.')
